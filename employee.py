...
def connect_database():
    try:
        connection = sqlite3.connect('inventory_system.db')  # SQLite database file
...
def treeview_data():
    ...
    employee_treeview.delete(*employee_treeview.get_children())  # Clear TreeView
...
def add_employee(empid, name, gender, email, number, dob, salary, address, usertype, password):
    if (empid == '' or name == '' or email == '' or number == '' or gender == 'Select Gender' or 
        ...
        try:
            ...
            if not cursor or not connection:
                return
            cursor.execute('''
                INSERT INTO employee_data (empid, name, gender, email, number, dob, salary, address, usertype, password)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
