...
- **Data integrity**: Foreign keys enforced (`PRAGMA foreign_keys=ON`) on every link (sales, purchases, returns, credit payments, and stock movements), UNIQUE constraints on category/product/customer names, CHECK constraints on prices/quantities, and all money stored as integer cents to avoid float drift.
...
- `employee_data` is rebuilt with typed columns; salaries parsed to numbers, dates converted to ISO format, passwords hashed. A backup is kept as `employee_data_old`.
...
- Schema version is tracked with `PRAGMA user_version` (currently 8). Upgrades add new columns/tables automatically (e.g. `stock_movements`, product `cost_price` and `reorder_level`, sales `invoice_number`/`payment_mode`, the `returns` and `purchases` tables, the `customers`/`credit_payments` tables, and the persisted login-lockout columns on `employee_data`). When upgrading, existing sales are back-filled into customer records automatically.
- **Money in integer cents**: all money columns (salaries, prices/costs, sale/purchase/return totals, credit limits, payments) are stored as `INTEGER` cents rather than floating-point `REAL`, avoiding float rounding errors. Databases at schema version 7 are rebuilt automatically on upgrade — each money value is rounded to the nearest cent (`ROUND(value * 100)`), and the rebuild also adds the missing foreign keys (`returns → sales/products`, `stock_movements → products`, `sales.customer_id → customers`, all `ON DELETE SET NULL`). Orphaned references are cleared before the rebuild and foreign-key enforcement is suspended during it; the whole migration is preceded by an automatic backup. A database whose money columns are already `INTEGER` is left untouched.
